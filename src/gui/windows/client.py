...
class ClientWindow(QWidget):
    ...
    def create_search_tab(self, tabs):
        ...
        self.p_last.setPlaceholderText("Last Name (Фамилия)")
        self.p_first = QLineEdit()
        self.p_first.setPlaceholderText("First Name (Имя)")
        self.p_middle = QLineEdit()
        self.p_middle.setPlaceholderText("Middle Name (Отчество - Optional)")
    ...
    def search_flights(self):
        ...
        try:
            ...
            for f in flights:
                ...
                if city_from and city_from not in dep_city:
                    continue
                if city_to and city_to not in arr_city:
                    continue

                self.cl_combo.addItem(
                    f"{f.flight_number} ({f.departure_airport.city.city_name} -> {f.arrival_airport.city.city_name})",
                    f.id
                )
        finally:
            session.close()
        ...
        try:
            ...
            if f.aircraft.photo_url:
                try:
                    ...
                    self.img.setText("Error loading image")
            else:
    ...
    def book(self):
        ...
        if not fid or not lname or not fname or not doc:
            QMessageBox.warning(self, "Validation", "Fill required fields!")
            return
